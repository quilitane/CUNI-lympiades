    ...
    <Card style={{ borderColor }} onClick={toggle}>
      <HeaderRow>
        <div style={{ display: 'flex', alignItems: 'center', color: textColor }}>
          ...
          <ArrowIcon $open={open} />
            <Title style={{ color: textColor, textDecoration: makeUncolored?"line-through":"none" }}>{challenge.name}</Title>
        </div>
        <Points style={{ color: textColor, textDecoration: makeUncolored?"line-through":"none" }}>{suspenseMode ? '???' : `${challenge.points} pts`}</Points>
      </HeaderRow>
